...
→ 브라우저 / Browser 확인
→ 레퍼런스 토큰 분석
→ AGENTS.md 규칙 + docs/design-backlog 백로그
→ 선택한 영역 하나 수정
→ 모바일 / build 검증
```
...
- `AGENTS.md`에는 규칙만 두고 `docs/design-backlog/`에 백로그를 남기는 방식
...
| 0:50-1:05 | 백로그 | AGENTS.md 규칙 추가, docs/design-backlog에 D-001 기록 |
| 1:05-1:35 | 실행 | D-001만 수정, 브라우저 확인 |
| 1:35-1:50 | 검증 | 390px 모바일, npm run build |
| 1:50-2:00 | 기록 | before/after, 다음 질문, design-backlog 상태 갱신 |
...
- docs/design-backlog의 D-001 상태가 갱신됐다.
